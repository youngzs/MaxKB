...
class ValidateDocxUploadTest(TestCase):
    def test_accepts_docx_with_correct_mime(self):
        f = _UploadStub("report.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        # Must not raise.
    ...
    def test_rejects_non_docx_extension(self):
        f = _UploadStub("report.pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
...
class UploadSerializerTest(TestCase):
    ...
    def test_rejects_unknown_scenario(self):
        f = _UploadStub("a.docx")
        serializer = DocumentTemplateUploadSerializer(data={"file": f, "name": "Hello", "scenario": "not-a-thing"})
...
class UpdateSerializerTest(TestCase):
    ...
    def test_placeholder_required_defaults_false(self):
        payload = [{"key": "borrower_name", "label": "借款人名称", "type": "text"}]
        serializer = DocumentTemplateUpdateSerializer(data={"placeholders": payload})
        self.assertTrue(serializer.is_valid(), serializer.errors)
        self.assertFalse(serializer.validated_data["placeholders"][0]["required"])

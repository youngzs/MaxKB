...
def auto_tag_documents(knowledge_id, documents: Iterable[Document]) -> int:
    ...
    for doc in documents:
        ...
        existing_tag_ids = set(
            str(tid) for tid in QuerySet(DocumentTag).filter(document_id=doc.id).values_list("tag_id", flat=True)
        )

        for key, value in tags_to_link:
            try:
                # savepoint 包住 get-or-create：tag.save() 冲突回滚自身，不毒化上传事务
            ...
            if str(tag.id) in existing_tag_ids:
                continue
            new_links.append(DocumentTag(id=uuid.uuid7(), document_id=doc.id, tag_id=tag.id))
            existing_tag_ids.add(str(tag.id))

    if new_links:
        try:
            # savepoint 包住批量写：失败回滚自身，不毒化外层 batch_save 上传事务
            with transaction.atomic():
                QuerySet(DocumentTag).bulk_create(new_links, ignore_conflicts=True)
        except Exception as e:  # noqa: BLE001
            maxkb_logger.warning(f"[auto_tag] bulk_create DocumentTag failed: {e}")
            return 0

...
> 状态：**已优化（配置层）**（2026-05-22）。

### 14.1 现象

A/B/C 全部上线后，chat-entry 问答出结果慢，实测一次问答 ~24s。
...
检索（A/B/C 那套）完全不是瓶颈，瓶颈是 LLM 调用。

### 14.3 根因

app 对话模型 name 显示「deepseek-v3」，但真实 `model_name` 是
**`deepseek-reasoner`**（DeepSeek 推理模型）—— 每次回答前生成长思维链，慢且不
计入最终答案（实测「21s 仅产出 522 字答案」）。查询拆解（C 档）也复用同一推理
模型。知识库问答本质是「上下文提取 + 组织」，不需要推理模型。

### 14.4 优化

把 app 对话模型（Application 本体 + 全部 3 个 ApplicationVersion 的 `model_id`）
从 `deepseek-reasoner` 换为 **`DeepSeek-V4-Flash`**（非推理快速模型）。配置层
改动、即时生效；主回答与查询拆解一并受益。

### 14.5 验证（2026-05-22）

换模型后实测同一查询：**合计 24s → 9.3s**（主 LLM 21s→6s，拆解+检索 ~7s→3s）。

### 14.6 后续可选优化

检索仅占 ~5%，下列收益相对有限，本次未做：

- 查询拆解加轻量预判：简单问题（短、无多意图词）跳过拆解 LLM 调用。
- C 档多路检索并行化（当前 N 路串行）。
- `max_paragraph_char_number` 适度回调，减少主 LLM prefill。
...
| v0.8 | 2026-05-22 | 新增 §十四：问答性能优化 —— 实测主 LLM 占 90% 耗时，根因是对话模型用了推理模型 `deepseek-reasoner`。换为 `DeepSeek-V4-Flash`（配置层），合计 24s→9.3s | Finance Workspace 小组 |

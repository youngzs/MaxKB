...
- 確保優化後的問題準確反映原始問題意圖，不得改變原意。`,
    ...
    docxRender: {
      title: 'DOCX 渲染',
      subtitle: '基於 docx 範本和佔位符值渲染輸出文件',
      templateKey: '範本 OSS Key',
      templateKeyPlaceholder: '請輸入範本檔案的 OSS Key（File.id）',
      values: '佔位符值',
      valuesPlaceholder: '請輸入 JSON 物件，例如 {"name": "World"}',
      valuesInvalid: '佔位符值必須是合法的 JSON 物件',
      outputFilename: '輸出檔名',
      outputFilenamePlaceholder: 'output.docx',
      outputOssKey: '輸出 OSS Key',
      error: '錯誤訊息',
    },
    zipPack: {
      title: 'ZIP 打包',
      subtitle: '把多個文件打包為一個 zip 檔案',
      documentKeys: '文件 OSS Key 列表',
      documentKeysPlaceholder: '每行一個 OSS Key（File.id）',
      documentKeysRequired: '請至少填寫一個文件 OSS Key',
      documentNames: '文件名稱（可選）',
      documentNamesPlaceholder: '每行一個名稱，與 OSS Key 列表按行對齊；留空使用預設名稱',
      itemFolders: '資料夾分組（可選）',
      itemFoldersPlaceholder: '每行一個資料夾名，與 OSS Key 列表按行對齊；留空則平鋪',
      archiveName: '輸出 zip 檔名',
      archiveNamePlaceholder: 'materials.zip',
      outputOssKey: '輸出 OSS Key',
      includedCount: '已包含檔案數',
      missing: '缺失檔案',
      error: '錯誤訊息',
    },
    documentSplitNode: {

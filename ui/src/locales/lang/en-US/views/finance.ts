export default {
  title: 'Finance Workspace',
  overview: 'Overview',
  documents: 'Process Documents',
  feasibility: 'Feasibility Assessment',
  progress: 'Progress Tracking',
  intel: 'News Feed',
  template: 'Templates',
  audit: 'Audit Log',
  workerHealth: {
    title: 'Celery',
    online: 'Online',
    offline: 'Offline',
    tasksRegistered: '{n} finance task(s)',
  },
  comingSoon: 'Coming soon',
  planning: 'Planned',
  planningDoc: {
    feedback: 'Send feedback',
    painPoints: 'Current pain points',
    capabilities: 'Capabilities you will get',
    flow: 'End-to-end flow',
    dependencies: 'Dependencies & prerequisites',
    notDoing: 'Out of scope for this phase',
    feasibility: {
      lead:
        'Given a potential financing deal, give the analyst a "Go / No-go / Need more material" call in under five minutes — with cited evidence — so dead-end applications get filtered before the team invests real effort.',
      painPoints: [
        'Each bank / trust / leasing co. has its own red lines; new hires take months to internalise them',
        'The same deal shape gets re-evaluated from scratch every time, nothing accumulates',
        "Group / sub-entity counterparties' eligibility lists are scattered in email and chat",
      ],
      capabilities: [
        {
          icon: '⚖️',
          title: 'Rule-based first-pass',
          desc: 'Built-in eligibility rules for banks, trusts, brokers, leasing cos., and policy banks. A hit produces a verdict.',
        },
        {
          icon: '📊',
          title: 'Cited evidence',
          desc: 'Every verdict links back to the regulation clause, the knowledge-base doc, or the historical deal that justifies it.',
        },
        {
          icon: '🧮',
          title: 'Gap-checklist',
          desc: '"Need more material" verdicts auto-generate a checklist and can spawn a materials-packaging task in one click.',
        },
        {
          icon: '🔀',
          title: 'Side-by-side comparison',
          desc: 'Run the same deal against N counterparties at once; output a comparison matrix (limit range / tenor / indicative rate / composite score).',
        },
      ],
      flow: [
        { title: 'Inputs', desc: 'Entity, amount, use, collateral, candidate counterparties' },
        { title: 'Rule + AI scoring', desc: 'Run rule engine and LLM in parallel' },
        { title: 'Verdict', desc: 'Go / No-go / Need more material with evidence' },
        { title: 'Hand-off', desc: 'Spawn materials task or document in one click' },
      ],
      dependencies: [
        'Knowledge base must contain regulation files and counterparty eligibility handbooks',
        'Optional: external credit / company-info APIs (rule-only mode if absent)',
        'Needs LLM model; falls back to pure-rule verdicts when not configured',
      ],
      notDoing: [
        'Does not replace the credit committee — only pre-screens',
        'Does not commit to specific rates / limits — all numbers are indicative',
      ],
    },
    progress: {
      lead:
        "Pull every in-flight financing deal, across departments and counterparties and multi-month timelines, into one board: stage, owner, key docs, risk flags — at a glance. The system auto-flags anomalies before anyone has to ask.",
      painPoints: [
        'Deals live across Excel, email and chat — statuses drift',
        'Stage stalls have no central owner; we discover them only when the monthly report comes due',
        'Execs asking for "all active financings, today" still triggers an ad-hoc spreadsheet pull',
      ],
      capabilities: [
        {
          icon: '🗺️',
          title: 'Project timeline',
          desc: 'Gantt-style view from origination to disbursement with owners, planned and actual dates.',
        },
        {
          icon: '🚨',
          title: 'Auto risk-flagging',
          desc: 'Stage-dwell time, document completeness, deadline proximity all roll into yellow / red flags.',
        },
        {
          icon: '🎯',
          title: 'Exec dashboard',
          desc: 'Total in-flight, average cycle, pass rate, counterparty mix — drill-down by entity and team.',
        },
        {
          icon: '📅',
          title: 'Deadline alerts',
          desc: 'Expiring documents, agreement renewals, covenant deliveries — owners get notified before the deadline.',
        },
      ],
      flow: [
        { title: 'Data sources', desc: 'Reuse existing project / materials / document data' },
        { title: 'Stage modelling', desc: 'Per-financing-type stage templates' },
        { title: 'Risk scoring', desc: 'Rules plus historical-cohort baselines' },
        { title: 'Surface', desc: 'Gantt + dashboard + deadline list' },
      ],
      dependencies: [
        'P1 (project / materials / documents) is the data backbone — already shipped',
        "Need each financing type's standard stage config defined with the business team",
      ],
      notDoing: [
        'Does not replace external progress reports filed with counterparties',
        'Does not force analysts to fill anything — transitions are system-side wherever possible',
      ],
    },
    intel: {
      lead:
        'Listen continuously to rate, policy and counterparty signals; match each item against active deal profiles; surface only what is relevant, on the channel that fits.',
      painPoints: [
        'Policy and rate signals spread across PBoC, CBIRC, local regulators, counterparty WeChat accounts — nobody can watch them all',
        'Same policy hits different deals differently; broadcast = ignored',
        'Missing a window (LPR move, special-bond quota) can cost real money',
      ],
      capabilities: [
        {
          icon: '🛰️',
          title: 'Multi-source ingest',
          desc: 'Regulator sites, RSS feeds from counterparty channels, custom sources — unified pipeline.',
        },
        {
          icon: '🎯',
          title: 'Profile match',
          desc: 'Route items based on the entity, industry, and financing type of in-flight deals.',
        },
        {
          icon: '🔔',
          title: 'Multi-channel push',
          desc: 'In-workspace inbox / email / Wecom, with importance-based routing.',
        },
        {
          icon: '📚',
          title: 'Knowledge accumulation',
          desc: 'Every pushed item lands in the knowledge base and is reusable by materials and feasibility modules.',
        },
      ],
      flow: [
        { title: 'Configure sources', desc: 'Admin onboards RSS / API / custom feeds' },
        { title: 'Ingest + dedupe', desc: 'Scheduled pull, title+body hash dedupe' },
        { title: 'Profile scoring', desc: 'Relevance score against active deals' },
        { title: 'Route', desc: 'Importance-aware channel selection' },
      ],
      dependencies: [
        'At least three stable external sources (commercial or open APIs)',
        'Embedding model for similarity matching (falls back to keyword hits when not configured)',
        'P1 and P2 data depth determines push precision — ship after both',
      ],
      notDoing: [
        'Not a general financial-news aggregator (Wind / Bloomberg already exist)',
        'No sentiment analysis (out of scope)',
      ],
    },
  },
  overviewDesc:
    'One-stop management for finance project materials, documents, progress, and compliance audit.',
  aiBanner: {
    message:
      'No AI model is configured for this workspace. Parsing, summary, and fill features will use placeholder content. Configure an LLM model under the "Models" menu.',
    dismiss: 'Don\'t show again',
    configureCta: 'Configure now',
  },
  cards: {
    project: {
      title: 'Project Management',
      desc: 'Manage finance project profiles, statuses, and linked knowledge bases.',
    },
    materials: {
      title: 'Materials Packaging',
      desc: 'Intelligently package materials from the knowledge base based on checklists, ready for review.',
    },
    documents: {
      title: 'Process Documents',
      desc: 'Auto-generate internal reports and committee briefs from templates.',
    },
    feasibility: {
      title: 'Feasibility Assessment',
      desc: 'Decision support based on regulations and company profile.',
    },
    progress: {
      title: 'Progress Tracking',
      desc: 'Aggregate project progress and automatically flag risks.',
    },
    intel: {
      title: 'News Feed',
      desc: 'Capture rates, policies and counterparty signals and route them to the right deals.',
    },
  },
  overviewKpis: {
    inFlight: 'In-flight projects',
    inFlightHint: 'Projects in preparing / materials / engaging stages (not landed or terminated)',
    pendingReview: 'Pending reviews',
    pendingReviewHint: 'Materials tasks awaiting approval',
    docsTotal: 'Documents generated',
    docsTotalHint: 'Cumulative process-document generations',
    sendsRecent: 'Sends · last 7 days',
    sendsRecentHint: 'Outbound deliveries logged in audit',
  },
  overviewSections: {
    entries: 'Modules',
    planning: 'Coming soon',
    planningDesc: 'The capabilities below are on the roadmap. Click any card for the full plan.',
    viewPlan: 'View plan',
  },
  action: {
    enter: 'Enter',
    disabled: 'Not available yet',
    back: 'Back to overview',
  },
  project: {
    title: 'Finance Projects',
    newProject: 'New Project',
    editProject: 'Edit Project',
    deleteConfirm:
      'Are you sure you want to delete project "{name}"? Deletion is recorded in the audit log.',
    deleteSuccess: 'Deleted successfully',
    empty: 'No projects yet. Click "New Project" in the top-right to get started.',
    emptyFiltered: 'No projects match your filters.',
    emptyState: {
      title: 'Start your first finance project',
      subtitle:
        'Group materials, documents, and progress under one project so the workspace can track everything end-to-end.',
      cta: 'Create your first project',
    },
    searchPlaceholder: 'Search by project name',
    detailPlaceholder: 'Project detail',
    backToList: 'Back to project list',
    knowledgeBasePending: 'Select linked knowledge bases (optional)',
    notSet: 'Not set',
    plannedStages: 'Stage Planned Dates',
    plannedStagesHint: 'Stages are listed by project type; fill in a planned completion date for each, or leave blank.',
    statusManagedHint: 'Status is managed automatically by stage progression.',
    columns: {
      name: 'Name',
      code: 'Code',
      type: 'Type',
      status: 'Status',
      targetAmount: 'Target Amount',
      region: 'Region',
      createdAt: 'Created At',
      actions: 'Actions',
    },
    actions: {
      view: 'View',
      edit: 'Edit',
      delete: 'Delete',
    },
    fields: {
      name: 'Name',
      code: 'Project Code',
      projectType: 'Project Type',
      targetAmount: 'Target Amount',
      currency: 'Currency',
      status: 'Status',
      region: 'Region',
      industryCode: 'Industry Code',
      knowledgeBaseIds: 'Linked Knowledge Bases',
      description: 'Description',
      counterparty: 'Counterparty',
      owner: 'Project Owner',
    },
    placeholders: {
      name: 'Enter project name',
      code: 'Lowercase letters, digits, hyphens recommended',
      projectType: 'Select project type',
      status: 'Select status',
      targetAmount: 'Enter target amount',
      region: 'e.g. Shanghai',
      industryCode: 'e.g. C39',
      description: 'Enter description',
      counterparty: 'e.g. CITIC Bank',
      owner: 'Defaults to the creator',
      plannedAt: 'Pick a planned date',
    },
    projectType: {
      bank_loan: 'Bank Loan',
      bond: 'Bond',
      trust: 'Trust',
      abs: 'ABS',
      other: 'Other',
    },
    statusOptions: {
      preparing: 'Preparing',
      materials: 'Materials Ready',
      engaging: 'Engaging',
      landed: 'Landed',
      terminated: 'Terminated',
    },
    statusFilterAll: 'All statuses',
    sections: {
      basic: 'Basic Info',
      knowledge: 'Linked Knowledge Bases',
      auditLog: 'Activity Log',
      auditLogPlaceholder: 'No activity recorded yet for this project. Every write (create, edit, stage transition, knowledge-base link change) is captured here and on the Audit Log page.',
    },
    validation: {
      nameRequired: 'Please enter a project name',
      nameTooLong: 'Name cannot exceed 200 characters',
      typeRequired: 'Please select a project type',
      codeFormat: 'Code may only contain letters, digits, and hyphens',
      codeTooLong: 'Code cannot exceed 64 characters',
    },
  },
  progressPage: {
    refresh: 'Refresh',
    empty: 'No projects yet. Create one under "Projects" to track its progress here.',
    filterType: 'Project Type',
    filterStatus: 'Status',
    filterOwner: 'Owner',
    allTypes: 'All types',
    allStatuses: 'All statuses',
    allOwners: 'All owners',
    today: 'Today',
    legendTitle: 'Legend',
    noPlan: 'Unscheduled',
    stageStatus: {
      pending: 'Pending',
      active: 'In Progress',
      done: 'Done',
      skipped: 'Skipped',
    },
    drawerTitle: 'Project Progress',
    currentStage: 'Current Stage',
    owner: 'Project Owner',
    counterparty: 'Counterparty',
    advance: 'Advance Stage',
    rollback: 'Roll Back Stage',
    advanceConfirm: 'Advance "{name}" to the next stage?',
    rollbackPromptTitle: 'Roll Back Stage',
    rollbackPrompt: 'Enter a reason for the rollback (logged to audit, optional)',
    advanceSuccess: 'Advanced to the next stage',
    rollbackSuccess: 'Rolled back to the previous stage',
    stageSaved: 'Stage updated',
    terminatedHint: 'Project is terminated; stage transitions are disabled',
    stageHeader: {
      stage: 'Stage',
      status: 'Status',
      planned: 'Planned',
      actual: 'Actual',
    },
    editStage: 'Edit Stage',
    plannedAt: 'Planned Completion',
    stageOwner: 'Stage Owner',
    note: 'Note',
    inheritOwner: 'Empty = inherit project owner',
    kpi: {
      inFlight: 'In-flight Projects',
      inFlightHint: '{total} projects total',
      inFlightAmount: 'In-flight Amount',
      avgCycle: 'Avg. Cycle',
      avgCycleUnit: 'days',
      passRate: 'Pass Rate',
      risk: 'At Risk',
      riskHint: '{red} red · {yellow} yellow',
    },
    risk: {
      none: 'Normal',
      yellow: 'At Risk',
      red: 'High Risk',
      dwell_yellow: 'Stage dwell exceeds planned duration',
      dwell_red: 'Stage dwell exceeds 1.5x planned duration',
      deadline_yellow: 'Less than 3 days to planned completion',
      deadline_red: 'Past the planned completion date',
      materials_failed: 'A linked materials task has failed',
    },
    alertsTitle: 'Risk Alerts',
    alertsEmpty: 'No risk items right now — all clear',
    activeStage: 'Current Stage',
    viewProject: 'View',
  },
  templateLib: {
    title: 'Templates',
    upload: 'Upload Template',
    newTemplate: 'Upload New Template',
    empty: 'No templates yet. Click "Upload Template" in the top-right to get started.',
    emptyFiltered: 'No templates match your filters.',
    emptyState: {
      title: 'Build your template library',
      subtitle:
        'Upload .docx templates so the workspace can auto-fill placeholders for reports, committee briefs, and other recurring documents.',
      cta: 'Upload your first template',
    },
    backToList: 'Back to template library',
    deleteConfirm:
      'Are you sure you want to delete template "{name}"? Previously generated documents are unaffected.',
    deleteSuccess: 'Deleted successfully',
    saveSuccess: 'Saved',
    uploadSuccess: 'Uploaded. Set placeholder metadata below.',
    scenario: {
      all: 'All',
      internal_report: 'Internal Report',
      meeting: 'Committee Brief',
      system_process: 'System Process',
      other: 'Other',
    },
    fields: {
      name: 'Template Name',
      scenario: 'Scenario',
      file: 'Template File',
      placeholderCount: 'Placeholders',
      version: 'Version',
      updatedAt: 'Updated',
      isActive: 'Active',
    },
    actions: {
      view: 'View',
      edit: 'Edit',
      delete: 'Delete',
    },
    placeholderEditor: {
      title: 'Placeholder Metadata',
      key: 'Placeholder',
      label: 'Display Label',
      type: 'Type',
      required: 'Required',
      aiHint: 'AI Hint',
      aiHintColumn: 'AI fill hint (optional)',
      aiHintTooltip: {
        intro:
          'This column only affects the "AI auto-fill" button — when filling manually you can ignore it (leave blank).',
        whatLabel: 'What it does:',
        what:
          'Gets folded into the system prompt for the LLM. e.g. "Use the full legal name including ‘Limited’" stops the model from outputting an abbreviation.',
        whatNotLabel: 'What it does NOT do:',
        whatNot:
          'It does not directly replace the placeholder — the final content is whatever the user types (or the AI generates) when producing the document.',
      },
      enumOptions: 'Options',
      enumOptionsHelp: 'Options (comma separated)',
      enumOptionsPlaceholder: 'e.g. Yes,No',
      aiHintPlaceholder:
        'Tell the AI what should go here in one sentence, e.g. "Describe why this project needs financing in one sentence."',
      suggest: 'AI Generate Labels/Hints',
      suggestSuccess: 'Generated labels and AI hints. Review them before saving.',
      save: 'Save Changes',
      empty: 'This template has no placeholders.',
    },
    detailDownload: 'Download this template',
    placeholderType: {
      text: 'Text',
      long_text: 'Long Text',
      number: 'Number',
      date: 'Date',
      enum: 'Enum',
    },
    uploadDialog: {
      title: 'Upload Template',
      fileTip: 'Only .docx files are supported, up to 10MB per file.',
      fileDrop: 'Drop the file here, or',
      fileClick: 'click to upload',
      submit: 'Upload',
      downloadSample: 'Download sample template (.docx)',
      helpExpand: 'Show placeholder syntax help',
      helpCollapse: 'Hide placeholder syntax help',
      help: {
        section1Title: '1. Single-variable substitution',
        section1Desc: ' — wrap the variable name in double curly braces with spaces:',
        section2Title: '2. Suffix decides the inferred field type',
        section2Desc: ' (you can change it later on the detail page):',
        section2NumberKeys: '*_amount / *_count / *_number / *_total',
        section2NumberType: 'number',
        section2DateKeys: '*_date / *_at / *_time / *_deadline',
        section2DateType: 'date',
        section2LongTextKeys: '*_desc / *_summary / *_analysis / *_notes / *_content',
        section2LongTextType: 'long_text',
        section2OtherKeys: 'anything else',
        section2OtherType: 'text',
        section3Title: '3. Conditional blocks',
        section3Desc:
          ' (keep the whole expression in the same font run, otherwise docxtpl can\'t parse across XML runs):',
        section4Title: '4. Loops',
        section4Desc:
          ' (prefer a flat list of strings — avoid dotted attribute access like item.name):',
        section5Title: '5. Filters',
        section5Desc: ' (standard Jinja):',
        warning:
          '⚠️ Advanced syntax (conditionals/loops/filters/attribute access) must live in a single Word run — select the whole expression in Word and reset to one font. Otherwise docxtpl splits across XML runs and you\'ll hit "unexpected \'.\'" style errors.',
      },
    },
    validation: {
      nameRequired: 'Please enter a template name',
      scenarioRequired: 'Please select a scenario',
      fileRequired: 'Please upload a .docx file',
      fileType: 'Only .docx files are supported',
      fileSize: 'File size cannot exceed 10MB',
      labelRequired: 'Please enter a display label',
    },
  },
  documentsLib: {
    title: 'Process Documents',
    newGeneration: 'New Generation',
    empty: 'No generation records yet.',
    emptyFiltered: 'No records match your filters.',
    emptyState: {
      title: 'Generate your first process document',
      subtitle:
        'Pick a template, fill placeholders, and the workspace will produce a polished .docx in seconds — ready for review and download.',
      cta: 'Start a new generation',
    },
    columns: {
      project: 'Project',
      template: 'Template',
      status: 'Status',
      createdAt: 'Created At',
      createdBy: 'Created By',
      actions: 'Actions',
    },
    status: {
      generating: 'Generating',
      pending_review: 'Pending Review',
      confirmed: 'Confirmed',
      revoked: 'Revoked',
      failed: 'Failed',
    },
    statusFilterAll: 'All statuses',
    projectFilterAll: 'All projects',
    actions: {
      preview: 'Preview',
      confirm: 'Confirm',
      revoke: 'Revoke',
      download: 'Download',
      delete: 'Delete',
    },
    previewTitle: 'Document Preview',
    previewDisclaimer: {
      banner:
        'This is a simplified preview — custom fonts, headers/footers, and complex table styling are stripped. Download the original to see the full formatting.',
      download: 'Download full .docx',
    },
    confirmTitle: 'Confirm this generation?',
    confirmDesc: 'Confirming moves it into the workflow; you can revoke it at any time.',
    confirmSuccess: 'Confirmed',
    revokeTitle: 'Revoke this generation?',
    revokeDesc: 'Revoked generations can be resubmitted later.',
    revokeSuccess: 'Revoked',
    downloadFallbackName: 'process-document.docx',
    wizard: {
      title: 'New Document Generation',
      backToList: 'Back to list',
      steps: {
        selectTemplate: 'Select template',
        selectProject: 'Select project',
        fillPlaceholders: 'Fill placeholders',
        previewGenerate: 'Preview & generate',
      },
      next: 'Next',
      prev: 'Previous',
      generate: 'Generate Now',
      generateSuccess: 'Generation submitted',
      aiFillOne: 'AI Fill',
      aiFillBusy: 'Generating...',
      aiFillAll: 'AI Fill All Long-Text Placeholders',
      aiFillAllBusy: 'Generating {n} field(s)...',
      aiFillSuccess: 'AI fill applied to {count} field(s)',
      aiFillOk: 'AI fill succeeded',
      aiFillFallback: 'AI could not generate "{label}" — placeholder stub kept.',
      aiFillSummary: 'Filled {success} field(s), skipped {skipped}',
      aiFillEmpty: 'No long-text placeholders are available for AI fill.',
      noTemplate: 'No templates available yet. Upload a .docx template in the template library to get started.',
      noTemplateTitle: 'No templates uploaded',
      noTemplateCta: 'Go to template library',
      noProject: 'No projects available.',
      summary: 'Generation Summary',
      summaryTemplate: 'Template',
      summaryProject: 'Project',
      summaryValues: 'Placeholder values',
      validation: {
        templateRequired: 'Please select a template',
        projectRequired: 'Please select a project',
        placeholderRequired: 'Please fill required placeholder: {label}',
      },
      // Built-in aliases used when a template was uploaded without admin
      // metadata (``label == key`` in DB). See zh-CN counterpart for the
      // full rationale. Each entry's English-side display reuses the
      // canonical field name; the form remains usable but admins should
      // still customise via the template detail page for polish.
      fieldAliases: {
        project_name: 'Project name',
        project_code: 'Project code',
        project_type: 'Financing type',
        project_description: 'Project background',
        applicant: 'Applicant',
        borrower_name: 'Borrower entity',
        counterparty: 'Counterparty (financial institution)',
        financing_type: 'Financing type',
        target_amount: 'Target amount',
        amount: 'Amount',
        currency: 'Currency',
        duration_months: 'Tenor (months)',
        expected_close_date: 'Expected close date',
        industry_code: 'Industry code',
        region: 'Region',
        risk_level: 'Risk level',
        collateral_summary: 'Collateral / guarantee summary',
        repayment_source_notes: 'Repayment source notes',
        materials_summary: 'Materials overview',
        fund_purpose: 'Use of funds',
        background: 'Background',
        risk_assessment: 'Risk assessment',
        repayment_source: 'Repayment source',
      },
    },
  },
  materials: {
    title: 'Materials Packaging',
    newTask: 'New Task',
    empty: 'No tasks yet. Click "New Task" in the top-right to get started.',
    emptyFiltered: 'No tasks match your filters.',
    emptyState: {
      title: 'Package your first set of materials',
      subtitle:
        'Paste or upload a requirement list and the workspace will match the right documents from your knowledge base — ready to review and send.',
      cta: 'Create your first task',
    },
    deleteConfirm: 'Delete task "{title}"?',
    deleteSuccess: 'Deleted successfully',
    list: {
      columns: {
        title: 'Title',
        project: 'Project',
        status: 'Status',
        items: 'Items',
        selected: 'Selected',
        createdAt: 'Created at',
        actions: 'Actions',
      },
      actions: {
        resume: 'Resume',
        view: 'View',
        delete: 'Delete',
      },
      projectFilterAll: 'All projects',
      statusFilterAll: 'All statuses',
    },
    status: {
      draft: 'Draft',
      parsing: 'Parsing',
      matching: 'Matching',
      pending_review: 'Pending review',
      approved: 'Approved',
      sent: 'Sent',
      rejected: 'Rejected',
      failed: 'Failed',
    },
    create: {
      title: 'New materials task',
      project: 'Project',
      titleField: 'Task title',
      mode: 'Requirement list input',
      pasteText: 'Paste text',
      uploadFile: 'Upload file',
      titlePlaceholder: 'Please enter a task title',
      requirementPlaceholder:
        'Example:\n1. Financial statements (past 3 years)\n2. Articles of association\n3. Shareholder meeting resolutions\n...',
      fileHint: 'Supports .txt / .docx / .pdf, ≤5MB',
      fileDrop: 'Drop file here, or',
      fileClick: 'click to upload',
      submit: 'Create & parse',
      success: 'Task created',
    },
    detail: {
      backToList: 'Back to list',
      requirementSection: 'Requirement list',
      taskInfo: 'Task info',
      actions: {
        reparse: 'Reparse',
        rematch: 'Rematch',
        summarize: 'Regenerate summary',
        pack: 'Pack',
        submit: 'Submit for review',
        approve: 'Approve',
        reject: 'Reject',
        download: 'Download zip',
      },
      pane1Title: 'Requirement items',
      pane1EditTrigger: 'Edit items',
      pane2Title: 'Matched documents',
      pane3Title: 'AI summary',
      requiredMark: '*',
      editDialog: {
        title: 'Edit requirement items',
        alert:
          'Add / remove / rewrite items here. **Saving does not re-match documents** — click "Re-match" after editing if you want fresh matches.',
        requirementTextLabel: 'Raw requirement text (optional, kept for archive + re-parse)',
        requirementTextPlaceholder: 'Leave blank to keep the existing text',
        itemsHeader: 'Items ({n})',
        addRow: 'Add an item',
        keyColumn: 'Key',
        keyPlaceholder: 'snake_case, should be unique',
        labelColumn: 'Display label',
        labelPlaceholder: 'e.g. Business license (duplicate)',
        descColumn: 'Description',
        descPlaceholder: 'Optional — extra context for the matcher and the reviewer',
        requiredColumn: 'Required',
        empty: 'No items yet — click "Add an item" above, or use "Re-parse" outside the dialog to auto-generate.',
        cancel: 'Cancel',
        save: 'Save',
        invalidMessage: 'Please fill in both key and label, and keep keys unique.',
        saveSuccess: 'Saved',
      },
      lockedTooltip: 'Beyond your clearance — cannot be selected',
      lockedBadge: 'Locked',
      addDoc: '+ Add document manually',
      emptyDoc: '← Pick a document in the middle pane to see the AI summary',
      emptyMatched:
        'No matched documents for this item — use the button below to add one manually.',
      emptyParsedItems:
        'Requirement list not parsed yet. Click "Reparse" in the top-right.',
      score: 'Score',
      removeFromSelection: 'Remove from selection',
      regenerateSummary: 'Regenerate summary',
      openInKnowledge: 'Open in knowledge base',
      rejectDialogTitle: 'Reject task',
      rejectReasonLabel: 'Reject comment',
      rejectReasonPlaceholder: 'Please enter a reject reason',
      approveConfirmTitle: 'Approve this task?',
      approveConfirmDesc:
        'Once approved the task will move to the Approved state.',
      approveSuccess: 'Approved',
      rejectSuccess: 'Rejected',
      submitSuccess: 'Submitted for review',
      packSuccess: 'Packaging complete',
      summarizeSuccess: 'Summaries regenerated',
      summarizeBusy: 'Generating...',
      summarizeFallback: 'AI could not regenerate the summary — placeholder stub kept.',
      downloadFallbackName: 'materials-package.zip',
      parsingMessage: 'AI is parsing the requirement list...',
      matchingMessage: 'Matching against the knowledge base...',
      progressHint: 'Usually takes 10-30 seconds. You can leave this page; status will refresh automatically.',
      failedTitle: 'Task failed',
      retry: 'Retry',
      selectionUpdateSuccess: 'Selection saved',
      manualPicker: {
        title: 'Add document manually',
        searchPlaceholder: 'Search knowledge base documents',
        empty: 'No matching document (or all already in the list)',
        noKb: 'This project has no linked knowledge base yet. Add one via "Edit project" first.',
        confirm: 'Add to this item',
        addedCount: '{n} document(s) added',
      },
    },
    validation: {
      projectRequired: 'Please select a project',
      titleRequired: 'Please enter a task title',
      titleTooLong: 'Title cannot exceed 200 characters',
      requirementRequired: 'Please paste a list or upload a file',
      fileSizeExceeded: 'File exceeds 5MB',
      fileTypeInvalid: 'Only .txt / .docx / .pdf files are supported',
      rejectReasonRequired: 'Please enter a reject reason',
    },
  },
  auditPage: {
    subtitle: 'Review all audited finance actions within this workspace (newest first).',
    reset: 'Reset filters',
    exportCsv: 'Export CSV',
    empty: 'No audit records yet',
    emptyFiltered: 'No audit records match the current filters',
    emptyState: {
      title: 'No audit records yet',
      subtitle:
        'As users act on projects, materials, templates, and SMTP configs, the audit trail will populate here automatically.',
    },
    filters: {
      targetType: 'Target type',
      action: 'Action',
      actorId: 'Actor ID',
      actorIdPlaceholder: 'Filter by exact user UUID',
      dateRange: 'Date range',
      dateFrom: 'From',
      dateTo: 'To',
      keyword: 'Keyword',
      keywordPlaceholder: 'Matches request path or params',
      allTargets: 'All target types',
      allActions: 'All actions',
    },
    columns: {
      time: 'Time',
      actor: 'Actor',
      action: 'Action',
      targetType: 'Target type',
      targetIdShort: 'Target ID',
      ip: 'IP',
      payload: 'Details',
    },
    fields: {
      userAgent: 'User-Agent: ',
      targetId: 'Target ID: ',
    },
    actions: {
      viewPayload: 'View payload',
      CREATE: 'Create',
      UPDATE: 'Update',
      DELETE: 'Delete',
      READ: 'Read',
      REVIEW_PASS: 'Review pass',
      REVIEW_REJECT: 'Review reject',
      SEND: 'Send',
      DOWNLOAD: 'Download',
    },
    targetTypes: {
      PROJECT: 'Project',
      MATERIALS_TASK: 'Materials task',
      DOC_TEMPLATE: 'Doc template',
      DOC_GENERATION: 'Doc generation',
      SMTP_CONFIG: 'SMTP config',
      OTHER: 'Other',
    },
    dialog: {
      title: 'Audit Payload',
    },
  },
  smtp: {
    title: 'SMTP configuration',
    hint: 'Outbound SMTP credentials. Passwords are encrypted at rest; plaintext is never returned by the API.',
    empty: 'No SMTP configs yet. Use the button above to add one.',
    testSuccess: 'Test email sent',
    testFailed: 'Test send failed',
    emptyState: {
      title: 'Configure your first SMTP account',
      subtitle:
        'Add an outbound email account so the workspace can send materials packages and notifications on your behalf.',
      cta: 'Configure your first SMTP',
    },
    actions: {
      add: 'New SMTP config',
      edit: 'Edit',
      delete: 'Delete',
      test: 'Test send',
    },
    columns: {
      name: 'Name',
      host: 'Host / port',
      fromEmail: 'From address',
      isDefault: 'Default',
      default: 'default',
      actions: 'Actions',
    },
    form: {
      name: 'Name',
      host: 'SMTP host',
      port: 'Port',
      username: 'Username',
      password: 'Password',
      passwordPlaceholder: 'Enter password',
      passwordKeepHint: 'Leave blank to keep existing password',
      fromEmail: 'From address',
      fromName: 'From display name',
      useTls: 'Enable STARTTLS',
      useSsl: 'Enable SSL (SMTPS)',
      isDefault: 'Set as default',
      testTo: 'Recipient for test email',
    },
    dialog: {
      createTitle: 'New SMTP config',
      editTitle: 'Edit SMTP config',
      testTitle: 'Test send',
      deleteTitle: 'Delete SMTP config',
      deleteConfirm: 'Delete "{name}"? Tasks will no longer be able to send via this account.',
    },
    validation: {
      nameRequired: 'Name is required',
      hostRequired: 'SMTP host is required',
      portRequired: 'Port is required',
      usernameRequired: 'Username is required',
      passwordRequired: 'Password is required',
      fromEmailInvalid: 'Please enter a valid email address',
      testToRequired: 'Recipient is required',
    },
  },
  emailTemplate: {
    title: 'Email templates',
    hint: 'Reusable subject / body templates for outbound emails.',
    empty: 'No templates yet. Use the button above to add one.',
    emptyState: {
      title: 'Create your first email template',
      subtitle:
        'Re-use a polished subject and body across recipients. Markdown is fully supported and rendered to HTML on send.',
      cta: 'Create your first template',
    },
    actions: {
      add: 'New template',
      edit: 'Edit',
      delete: 'Delete',
    },
    columns: {
      name: 'Name',
      subject: 'Subject',
      scenario: 'Scenario',
      isActive: 'Status',
      active: 'Active',
      inactive: 'Inactive',
      actions: 'Actions',
    },
    scenario: {
      materials: 'Materials',
      progress_report: 'Progress report',
      general: 'General',
      other: 'Other',
    },
    form: {
      name: 'Name',
      subject: 'Subject',
      scenario: 'Scenario',
      bodyText: 'Plain-text body',
      bodyHtml: 'HTML body (optional)',
      bodyHtmlPlaceholder: 'Leave blank to send plain text only',
      markdownHint:
        'Markdown supported: **bold**, *italic*, lists, links — rendered to HTML on send.',
      isActive: 'Active',
      placeholderHint:
        'Available variables: {project_name}, {task_title}, {recipient_name}, {zip_filename}',
    },
    dialog: {
      createTitle: 'New email template',
      editTitle: 'Edit email template',
      deleteTitle: 'Delete email template',
      deleteConfirm: 'Delete "{name}"?',
    },
    validation: {
      nameRequired: 'Name is required',
      subjectRequired: 'Subject is required',
      bodyRequired: 'Body is required',
    },
  },
  send: {
    actions: {
      send: 'Send',
    },
    sendSuccess: 'Email sent',
    sendFailed: 'Send failed',
    sentTitle: 'Send Result',
    sentAll: 'Successfully sent to {n} recipient(s)',
    sentFailed: 'Send failed',
    retry: 'Retry',
    dialog: {
      title: 'Send materials package',
    },
    form: {
      to: 'To',
      toPlaceholder: 'Type an email and press Enter; add multiple',
      cc: 'Cc',
      ccPlaceholder: 'Type an email and press Enter; add multiple',
      smtp: 'SMTP account',
      smtpPlaceholder: 'Pick an SMTP config',
      template: 'Email template',
      templatePlaceholder: 'Pick a template',
      attachZip: 'Attach materials zip',
    },
    preview: {
      title: 'Preview',
      subject: 'Subject:',
    },
    validation: {
      toRequired: 'At least one recipient is required',
      toInvalid: 'One or more recipients are invalid',
      smtpRequired: 'SMTP config is required',
      templateRequired: 'Template is required',
    },
    log: {
      title: 'Send history',
      empty: 'No send history',
      columns: {
        createdAt: 'Time',
        to: 'To',
        subject: 'Subject',
        status: 'Status',
        error: 'Error',
      },
      status: {
        queued: 'Queued',
        sending: 'Sending',
        sent: 'Sent',
        failed: 'Failed',
        retried: 'Retried',
      },
    },
  },
  workflowRun: {
    title: 'Run log',
    openLogLink: 'View run log',
    refresh: 'Refresh',
    autoRefresh: 'Auto-refresh',
    totalCount: '{n} run(s)',
    empty: 'No runs yet',
    columns: {
      task: 'Task',
      status: 'Status',
      startedAt: 'Started',
      duration: 'Duration',
      retries: 'Retries',
      error: 'Error',
      actions: 'Actions',
    },
    status: {
      queued: 'Queued',
      running: 'Running',
      succeeded: 'Succeeded',
      failed: 'Failed',
      retrying: 'Retrying',
      cancelled: 'Cancelled',
    },
    retry: 'Retry',
    cancel: 'Cancel',
    cancelDismiss: 'Keep running',
    cancelConfirm:
      'Cancel this run? The task will be revoked and the related item marked as failed.',
    retrySuccess: 'Retry dispatched',
    retryFailed: 'Failed to dispatch retry',
    cancelSuccess: 'Run cancelled',
    cancelFailed: 'Failed to cancel run',
    staleBadge: '⚠ Possibly stuck',
    staleHint:
      'This run has been in flight for over 15 minutes — the worker may have been lost.',
    engine: 'Engine',
    nodeProgress: 'Node progress',
    noDetail: 'No execution detail recorded',
    inputsSummary: 'Inputs summary',
    outputsSummary: 'Outputs summary',
  },
}

...
- 确保优化后的问题准确反映原始问题意图，不得改变原意。`,
    ...
    docxRender: {
      title: 'DOCX 渲染',
      subtitle: '基于 docx 模板和占位符值渲染输出文档',
      templateKey: '模板 OSS Key',
      templateKeyPlaceholder: '请输入模板文件的 OSS Key（File.id）',
      values: '占位符值',
      valuesPlaceholder: '请输入 JSON 对象，例如 {"name": "World"}',
      valuesInvalid: '占位符值必须是合法的 JSON 对象',
      outputFilename: '输出文件名',
      outputFilenamePlaceholder: 'output.docx',
      outputOssKey: '输出 OSS Key',
      error: '错误信息',
    },
    zipPack: {
      title: 'ZIP 打包',
      subtitle: '把多个文档打包为一个 zip 文件',
      documentKeys: '文档 OSS Key 列表',
      documentKeysPlaceholder: '每行一个 OSS Key（File.id）',
      documentKeysRequired: '请至少填写一个文档 OSS Key',
      documentNames: '文档名称（可选）',
      documentNamesPlaceholder: '每行一个名称，与 OSS Key 列表按行对齐；留空使用默认名',
      itemFolders: '文件夹分组（可选）',
      itemFoldersPlaceholder: '每行一个文件夹名，与 OSS Key 列表按行对齐；留空则平铺',
      archiveName: '输出 zip 文件名',
      archiveNamePlaceholder: 'materials.zip',
      outputOssKey: '输出 OSS Key',
      includedCount: '已包含文件数',
      missing: '缺失文件',
      error: '错误信息',
    },
    documentSplitNode: {
